...
function handleKeyboardInput(event) {
    ...
    if (!isNaN(key) || key === ',') {
        ...
        clearDisplay(); // esc limpa o campo display
    } else if (event.ctrlKey && (key === 'p' || key === 'P')) {
        printResult(); //atalho para imprimir
        event.preventDefault();
    } else if (event.shiftKey && (key === 'w' || key === 'W')) {
        clearDescription(); //atalho para limpra o campo descrição
        event.preventDefault();
    } else if (event.shiftKey && (key === 'd' || key === 'D')) {
        selectDescription(); //atalho para selecionar o campo descrição
        event.preventDefault();
    }
}
